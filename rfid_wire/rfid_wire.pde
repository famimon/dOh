...
// empty the array of found items every 1/2 minute
unsigned long FOUND_TIMEOUT = 30000;

// Create new set when registering item if no reference set is found in 2 seconds
unsigned long REGISTER_TIMEOUT = 2000;

byte BEEPER_TAG[] = {0xE2, 0x0, 0x10, 0x66, 0x66, 0x14, 0x0, 0x76, 0x19, 0x40, 0x4C, 0xCC};
byte ID_TAG[] = {0xE2, 0x0, 0x10, 0x66, 0x66, 0x14, 0x0, 0x74, 0x19, 0x40, 0x4C, 0xC4};
byte PHONE_TAG[] = {0xE2, 0x0, 0x10, 0x66, 0x66, 0x14, 0x0, 0x75, 0x19, 0x40, 0x4C, 0xC8};
byte DOCUMENT_TAG[] = {0x30, 0x8, 0x33, 0xB2, 0xDD, 0xD9, 0x6, 0xC0, 0x0, 0x0, 0x0, 0x0};
byte HOME_TAG[] = {0xE2, 0x0, 0x19, 0x96, 0x96, 0x2, 0x1, 0x32, 0x21, 0x50, 0x35, 0x1D};
byte CAR_TAG[] = {0xE2, 0x0, 0x19, 0x96, 0x96, 0x2, 0x1, 0x32, 0x10, 0x90, 0xA9, 0x17};
...
const byte ssRX = 9;
// Connect Arduino pin 4 to RX of usb-rfid device
const byte ssTX = 4;
...
int toCheck[MAXSETS]; //Array storing  which sets to look for missing

boolean deleting = false;
boolean registering = false;
//It's true until first item found
boolean none = true;
...
{  
  ...
  // found should have a vailidy of about 30 seconds
  ...
  if(none){
    ...
    for (int s=0; s < nSets; s++){
      for (int m=0; m < lengths[s]; m++){
        if(toCheck[s] && !metaInventory[s][m].found){
          buffer[i++] = metaInventory[s][m].type;          
        }
      }
    }
  }
...
void registerItem(byte newTag[]){
  ...
  int type = checkType(newTag);
  if(referenceSet==-1){
    for (int i=0; i<TAGSIZE; i++){ // creates set with single item
      inventory[nSets][0][i]=newTag[i];
    }
    metaInventory[nSets][0].type=type;
    metaInventory[nSets][0].found=false;
    lengths[nSets++]=1;
  }
    ...
    metaInventory[referenceSet][lengths[referenceSet]].type=type;
    metaInventory[referenceSet][lengths[referenceSet]].found=false;    
...
int checkType(byte tag[]){
  if (sameTag(tag, BEEPER_TAG)){
    return BEEPER_TYPE;
  }
  if (sameTag(tag, ID_TAG)){
    return ID_TYPE;
  }
  if (sameTag(tag, PHONE_TAG)){
    return PHONE_TYPE;
  }
  if (sameTag(tag, DOCUMENT_TAG)){
    return DOC_TYPE;
  }
  if (sameTag(tag, HOME_TAG)||sameTag(tag, CAR_TAG)){
    return KEY_TYPE;
  }
}
...
void deleteItem(int set, int idx){
  displayType(checkType(inventory[set][idx]));
